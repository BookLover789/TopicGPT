...
class TopwordEnhancement:
    ...
    def __init__(
        ...
        Args:
            ...
            model_temperature (float, optional): The softmax temperature to use for the OpenAI model (default is 0.5).
        ...
        assert model is not None, "Please provide an openai model"
        assert max_context_length > 0, "Please provide a positive max_context_length"
        assert model_temperature > 0, "Please provide a positive model_temperature"
    ...
    def __str__(self) -> str:
        repr = f"TopwordEnhancement(model = {self.model})"
        return repr
    ...
    def count_tokens_api_message(self, messages: list[dict[str]]) -> int:
        ...
        """
        return self.client.count_tokens(messages, model=self.model)

    def describe_topic_topwords_completion_object(
                                self, 
                                topwords: list[str], 
                                n_words: int = None,
        ...
        if provider == "openai":
            ...
            tokens_cumsum = np.cumsum([len(tiktoken.encoding_for_model(self.model).encode(tw + ", ")) for tw in topwords]) + len(tiktoken.encoding_for_model(self.model).encode(self.basic_model_instruction + " " + self.corpus_instruction))
            ...
            # Use word count or skip token counting
            prompt = self.basic_model_instruction + " " + self.corpus_instruction + "\n" + query_function(topwords)
            completion = self.client.chat_completion(
                model=self.model,
            ...
            completion = self.client.chat_completion(
                model=self.model,
                messages=[{"role": "user", "content": query_function(topwords)}],
                temperature=self.model_temperature
            )
            return completion
        else:
    ...
    def describe_topic_topwords_str(self, 
                                topwords: list[str], 
                                n_words: int = None,
                                query_function: Callable = lambda tws: f"Please give me the common topic of those words: {tws}. Also describe the various aspects and sub-topics of the topic. Make sure the descriptions are short and concise! Do not cite more than 5 words per sub-aspect!!!"
                            ) -> str:
        """
        Describe the given topic based on its topwords using the OpenAI model.
        ...
        return self.client.get_str_from_completion_object(completion)
    ...
    def describe_topic_documents_completion_object(
                                            self, 
                                            documents: list[str],
        ...
        tokens_cumsum = np.cumsum([len(tiktoken.encoding_for_model(self.model).encode(doc + ", ")) for doc in documents]) + len(tiktoken.encoding_for_model(self.model).encode(self.basic_model_instruction + " " + self.corpus_instruction))
        if tokens_cumsum[-1] > self.max_context_length:
